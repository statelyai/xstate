...
With the xstate interpreter, delayed actions will use the native`setTimeout` and `clearTimeout` functions:

```js
import { interpret } from 'xstate/lib/interpreter';

const interpreter = interpret(lightDelayMachine)
...
For testing, xstate provides a `SimulatedClock`:
...
interpreter.clock.increment(1000);
// => 'yellow'
```
