    ...
    red: {
      ...
      states: {
        ...
        crosswalkSouth: {
          ...
          onDone: {
            // 'stop' is a final state node for 'crosswalkSouth'
            actions: 'stopCrosswalkSouth'
          }
        }
      },
      onDone: 'green'
...
In a compound state, reaching a final child state node (with `{ type: 'final' }`) will interally raise a `done(...)` event for that compound state node (e.g., `"done.state.light.crosswalkSouth"`). Using `onDone` is equivalent to defining a transition for this event.
