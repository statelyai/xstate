...
| `seed` | `0` | Seed for the sampling. Each event case draws from its own derived stream, so adding a case leaves the other cases' payloads unchanged. `outcomes` draws from its own streams too. |
...
The shared options include `mode` and `outcomes`. Traversal routes through the
machine's internal transitions — `xstate.done.actor`, `xstate.error.actor`,
and `xstate.after` — so the invoke and timer branches are covered from here
too. See [Choosing between path testing and property
testing](#choosing-between-path-testing-and-property-testing) for how each
mode drives them.
...
- Invoked actors never start, so their `onDone`, `onError`, and `onSnapshot`
  ...
  transitions. `testPaths()` does this for you: it sends the
  `xstate.done.actor` and `xstate.error.actor` events its paths went through,
  with the payload sampled from `outcomes`.
...
The lower-level path helpers (`getShortestPaths()`, `getSimplePaths()`, and the
other `TestModel` path methods) offer only the events you give them, so
transitions that depend on invoked actors or timers are not discovered unless
you supply those events yourself, or use `getPathsFromEvents()` with an event
sequence you control. `testPaths()` offers them for you.
...
moves only through `advance` commands — generated by `propertyTest()`, or
derived by `testPaths()` from the `xstate.after` steps its paths took — and no
`sut` is required for them:
...
`actors` and `outcomes` are rejected in pure mode by `propertyTest()`.
`testPaths()` accepts `outcomes` in either mode: in pure mode it samples the
outcomes into the `xstate.done.actor` and `xstate.error.actor` events it
sends, and in executed mode it stubs each source a path resolves — whether or
not `outcomes` names it — and resolves the stub with the sampled outcome for
the branch the path took, or with a synthesized one when there is none.
...
| `after` and invoked actors | Traversed; `mode: 'executed'` drives them | `mode: 'executed'` with `advance`/`outcomes` |
| Offline suites | — | `generateTestSuite()` |

Both entry points reach `onDone`, `onError`, and `after` transitions. The
state graph carries them as internal transitions, so traversal routes through
them, and each entry point turns that into a run differently:

- In `mode: 'pure'`, `testPaths()` sends the internal event itself —
  `xstate.done.actor` carries the `output` sampled from `outcomes`,
  `xstate.error.actor` the `error`, and `xstate.after` no payload. Nothing
  actually runs.
- In `mode: 'executed'`, the same step becomes the command that produces it
  against a real actor: an `outcome` command resolving the stubbed invoke
  source, or an `advance` of the delay. `propertyTest()` generates those
  commands instead of deriving them from a path.

`testPaths()` accepts `outcomes` and samples it like `events`. It still
rejects `commands`, which configures how `propertyTest()` generates runtime
commands; paths decide their own.
