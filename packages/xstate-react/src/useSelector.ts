...
const defaultGetSnapshot = <TActor extends ActorRef<any, any>>(a: TActor) =>
  'getSnapshot' in a
    ? a.getSnapshot()
    : isService(a)
...
>(
  ...
  const subscription = useMemo(() => {
    ...
    // - `compare`: is really supposed to be idempotent and the same throughout the
    // lifetime of this hook (the same assumption is made in React Redux v7)
  }, [actor]);
