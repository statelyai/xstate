...
>(
  ...
  const next = childStateNode.next(snapshot, event, actorRef);

  if (!next || !next.length) {
    return stateNode.next(snapshot, event, actorRef);
  }
...
function selectTransitions(
  ...
  return nextState.machine.getTransitionData(nextState as any, event, actorRef);
}
