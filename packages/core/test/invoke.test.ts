...
import { spawnActivity, spawnMachine } from '../src/invoke';
...
describe('invoke', () => {
  ...
  describe('with callbacks', () => {
    ...
    it('should be able to be stringified', () => {
      ...
      expect(typeof waitingState.actions[0].def.src).toBe('string');
    });
    ...
    it('should sync with child machine when sync: true option is provided', done => {
      ...
      const machine = Machine({
        ...
        states: {
          pending: {
            invoke: {
              src: spawnMachine(childMachine, { sync: true }) // TODO: consider adding sync option to invoke
            }
          },
          success: { type: 'final' }
        }
      ...
      const service = interpret(machine).onTransition(state => {
        if (state.event.type === actionTypes.update) {
          expect((state.event as UpdateObject).state.context).toEqual({
            count: 42
          });
          done();
        }
