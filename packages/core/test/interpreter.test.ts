...
describe('interpreter', () => {
  ...
  it('should not update when stopped', () => {
    ...
    expect(console.warn).toMatchMockCallsInlineSnapshot(`
      [
        [
          "Event "TIMER" was sent to stopped actor "machine:0 (machine:0)". This actor has already reached its final state, and will not transition.
      Event: {"type":"TIMER"}",
        ],
      ]
    `);
    ...
    it('should not execute transitions after being stopped', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(console.warn).toMatchMockCallsInlineSnapshot(`
          [
            [
              "Event "TRIGGER" was sent to stopped actor "machine:0 (machine:0)". This actor has already reached its final state, and will not transition.
          Event: {"type":"TRIGGER"}",
            ],
          ]
        `);
