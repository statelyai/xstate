...
- Machines are serializable again: `machine.definition` and `machine.toJSON()` return the JSON-safe definition (so `JSON.stringify(machine)` never throws). Inline functions, actor logic, and runtime schemas appear as explicit `{ "$unserializable": ... }` markers. `createMachineFromConfig(json)` is now exported from `xstate` and round-trips losslessly with `JSON.stringify(machine)`:

  ```ts
  import { createMachineFromConfig } from 'xstate';

  const revived = createMachineFromConfig(JSON.parse(JSON.stringify(machine)));
  ```
