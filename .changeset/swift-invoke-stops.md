...
Pure transition functions also accept an optional `ActorSystemRuntime`. It can override actor lifecycle, messaging, emitted events, and timer operations while the default actor system requires no configuration. `executeEffects(...)` executes returned effects sequentially and awaits asynchronous runtime operations.
