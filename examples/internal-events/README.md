...
`send` is fire-and-forget. A rejected event does not throw at the call site and does not error the actor — the actor's status stays `active`. The rejection is only visible if you observe it: `onRejectedEvent` receives an `EventRejection` with the `event`, `reason` (`'internalEvent'` here), `eventOrigin` and `error`.
