...
Add water on a timer until the configured maximum is reached.

Run `pnpm --filter @xstate/example-workflow-filling-water start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
