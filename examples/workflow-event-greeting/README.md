...
Wait for a greeting event and return the asynchronous greeting result.

Run `pnpm --filter @xstate/example-workflow-event-greeting start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
