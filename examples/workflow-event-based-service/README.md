...
Create a simulated veterinary appointment and return to idle with its result.

Run `pnpm --filter @xstate/example-workflow-event-based-service start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
