...
Pass the input person to an asynchronous greeting operation and return its result.

Run `pnpm --filter @xstate/example-workflow-greeting start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
