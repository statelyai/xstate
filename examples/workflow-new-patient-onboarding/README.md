...
Store patient information, assign a doctor, and schedule an appointment with retryable simulated operations.

Run `pnpm --filter @xstate/example-workflow-new-patient-onboarding start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
