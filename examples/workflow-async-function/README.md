...
Wait for a simulated email operation before completing.

Run `pnpm --filter @xstate/example-workflow-async-function start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
