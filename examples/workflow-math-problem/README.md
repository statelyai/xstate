...
Run simulated math tasks concurrently and collect every result.

Run `pnpm --filter @xstate/example-workflow-math-problem start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
