...
Process order events or cancel the order after its deadline.

Run `pnpm --filter @xstate/example-workflow-purchase-order-deadline start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
