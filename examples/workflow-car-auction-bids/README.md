...
Collect bids during the auction window and return the highest bid. An empty auction completes with no winning bid.

Run `pnpm --filter @xstate/example-workflow-car-auction-bids start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
