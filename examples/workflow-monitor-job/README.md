...
Submit a job, poll its status, and report success or failure.

Run `pnpm --filter @xstate/example-workflow-monitor-job start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
