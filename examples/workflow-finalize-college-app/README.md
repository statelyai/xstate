# Finalize college app

Wait for all three application documents before finalizing the application.

Run `pnpm --filter @xstate/example-workflow-finalize-college-app start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
