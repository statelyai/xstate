...
Collect temperature and humidity readings, produce a report, and begin the next collection window.

Run `pnpm --filter @xstate/example-workflow-accumulate-room-readings start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
