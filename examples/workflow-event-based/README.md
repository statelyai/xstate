...
Select the approved, rejected, or timed-out visa workflow from incoming events.

Run `pnpm --filter @xstate/example-workflow-event-based start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
