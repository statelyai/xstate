# Temperature Converter

This is a temperature converter built with:

- XState v6 alpha
- Vue
- TypeScript
- Vite

It's the 2nd task of the [7 GUIs](https://eugenkiss.github.io/7guis).

## Demo Video

[7GUIs -- cute -- 2.webm](https://github.com/tsxoxo/xstate/assets/59713582/313e6b07-97d7-4c6c-99cb-ee376fa03267)

## Live

[Open in CodeSandbox](https://codesandbox.io/p/sandbox/github/statelyai/xstate/tree/next/examples/7guis-2-temperature-vue)

[![Open in StackBlitz](https://developer.stackblitz.com/img/open_in_stackblitz.svg)](https://stackblitz.com/github/statelyai/xstate/tree/next/examples/7guis-2-temperature-vue)

<!-- sync:src -->

This example uses the workspace XState v6 alpha. Machine schemas use `types<T>()` for static typing. From the repository root, install dependencies and run `pnpm build` first, then:

```sh
pnpm --dir examples/7guis-2-temperature-vue dev
pnpm --dir examples/7guis-2-temperature-vue build
```

The machine regression tests run with the repository's `pnpm check:examples` command.
