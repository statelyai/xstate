...
Dispatch the appropriate patient action for each incoming vital-sign event.

Run `pnpm --filter @xstate/example-workflow-monitor-patient start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
