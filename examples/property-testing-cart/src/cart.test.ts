...
describe('cart', () => {
  ...
  it('replays a recorded counterexample', async () => {
    ...
    try {
      ...
      fixture = (error as PropertyTestFailure).fixture;
    }

    expect(fixture).toBeDefined();

    // The fixture is plain JSON: commit it and the same failure is reproduced
    // without generating anything. `replayPropertyTest()` expects the recorded
    // failure to happen again, and throws `PropertyReplayNotReproducedError`
    // when it does not.
    const reproduced = replayPropertyTest(cartMachine, fixture!, {
      sut: {
        create: () => {
          const store = new CartStore();
          return {
    ...
    await expect(reproduced).rejects.toBeInstanceOf(PropertyTestFailure);
    delete process.env.CART_BUG;
  });
});
