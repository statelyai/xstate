...
Poll the inbox and simulate text notifications for high-priority messages. Stopping the actor clears the polling interval.

Run `pnpm --filter @xstate/example-workflow-check-inbox start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
