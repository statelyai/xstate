...
Wait for both parallel asynchronous branches before completing.

Run `pnpm --filter @xstate/example-workflow-parallel start` from the repository root. `build` checks this Node example's TypeScript; it does not create a browser bundle. Run `pnpm build` at the root first to generate XState declarations.
